...
class TradingAgentsGraph:
    ...
    def __init__(
        ...
        self.curr_pretty_texts: List[str] = []
    ...
    def _create_tool_nodes(self) -> Dict[str, ToolNode]:
        ...
        Respect configuration flag `online_tools`:
        ...
        # News tools (Google News scraping is online; keep it only in online mode)
        if online:
            news_tools = [
    ...
    def propagate(self, company_name, trade_date):
        ...
        if self.debug:
            ...
            trace = []
            pretty_texts: List[str] = []
            for chunk in self.graph.stream(init_agent_state, **args):
                if len(chunk["messages"]) == 0:
                    pass
                else:
                    chunk["messages"][-1].pretty_print()
                    pretty_texts.append(self._message_to_text(chunk["messages"][-1]))
                    trace.append(chunk)

            final_state = trace[-1]
            self.curr_pretty_texts = pretty_texts
        else:
    ...
    def _log_state(self, trade_date, final_state):
        ...
        # Save to file（使用配置的 results_dir，避免当前工作目录与中文路径问题）
        base_results_dir = Path(self.config.get("results_dir", "./results"))
        directory = base_results_dir / str(self.ticker) / "TradingAgentsStrategy_logs"
        directory.mkdir(parents=True, exist_ok=True)

        out_path = directory / f"full_states_log_{trade_date}.json"
